...
def map_to_cord(pose_map, threshold=0.1):
    ...
    y, x, z = np.where(
        np.logical_and(
            pose_map == pose_map.max(axis=(0, 1)), pose_map > threshold
        )
    )
...
def cords_to_map(cords, img_size, old_size=None, affine_matrix=None, sigma=6):
    ...
    for i, point in enumerate(cords):
        ...
        if affine_matrix is not None:
            point_ = np.dot(
                affine_matrix, np.matrix([point[1], point[0], 1]).reshape(3, 1)
            )
        ...
        result[..., i] = np.exp(
            -((yy - point_0) ** 2 + (xx - point_1) ** 2) / (2 * sigma**2)
        )
    return result
...
def draw_pose_from_cords(pose_joints, img_size, radius=2, draw_joints=True):
    ...
    if draw_joints:
        for f, t in LIMB_SEQ:
            from_missing = (
                pose_joints[f][0] == MISSING_VALUE
                or pose_joints[f][1] == MISSING_VALUE
            )
            to_missing = (
                pose_joints[t][0] == MISSING_VALUE
                or pose_joints[t][1] == MISSING_VALUE
            )
    ...
    for i, joint in enumerate(pose_joints):
        if (
            pose_joints[i][0] == MISSING_VALUE
            or pose_joints[i][1] == MISSING_VALUE
        ):
            continue
...
def draw_legend():
    ...
    plt.legend(
        handles=handles, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0
    )
...
def produce_ma_mask(kp_array, img_size, point_radius=4):
    ...
    for f, t in limbs:
        ...
        to_missing = (
            kp_array[t][0] == MISSING_VALUE or kp_array[t][1] == MISSING_VALUE
        )
        if from_missing or to_missing:
            continue
    ...
    for i, joint in enumerate(kp_array):
        if kp_array[i][0] == MISSING_VALUE or kp_array[i][1] == MISSING_VALUE:
            continue
        yy, xx = ellipse(
            joint[0], joint[1], point_radius, point_radius, shape=img_size
        )
        mask[yy, xx] = True
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        mmm = (
            produce_ma_mask(pose_cords, (128, 64))
